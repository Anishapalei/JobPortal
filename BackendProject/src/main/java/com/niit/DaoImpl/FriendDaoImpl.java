...
public class FriendDaoImpl implements FriendDao {
	...
	public List<User> suggestedUsers(String email) {
		...
		String queryString = "select * from newuser_s190020 where email in"
				+ "(select email from newuser_s190020 where email!=?" + "minus"
				+ "(select toId_email from newfriend_s190020 where fromId_email=?"
				+ "union select fromId_email from newfriend_s190020 where toId_email=?))";
